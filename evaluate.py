...
import argparse
import time
from detectron2 import Object_Detect
...
for k,(imgs) in enumerate(test_batch):
    ...
    with torch.no_grad():
        ...
        rcnnimg,rcnnm,mmmmm= Object_Detect.plotfun(test_dataset.samples[k])

    original_img= cv2.imread(test_dataset.samples[k])
    original_mask= cv2.imread(test_dataset.samples[k])


    lose_AE= loss_func_mse(outputs[0, :, 8], imgs[0, :, 8])
    lose_AE = lose_AE.detach().cpu().numpy()

    cn_image_correct = (lose_AE - lose_AE.min()) / (lose_AE.max()- lose_AE.min()) 
    rcnnm=rcnnm/255 
    concat_rcnnm= cv2.merge((rcnnm,rcnnm,rcnnm))

    averaged_final_mask=np.add(cn_image_correct,rcnnm)/2

    only_MSE= lose_AE[:,:,0]

    loss_pixel = torch.mean(torch.from_numpy(averaged_final_mask))
